...
: rclcpp::Node("tarkbot_robot")
{
  ...
  imu_frame_ = get_parameter("imu_frame").get_parameter_value().get<std::string>();

  //话题消息初始化
  declare_parameter("odom_topic", "odom");
  odom_topic_ = get_parameter("odom_topic").get_parameter_value().get<std::string>();
  declare_parameter("imu_topic", "imu");
  imu_topic_ = get_parameter("imu_topic").get_parameter_value().get<std::string>();
  declare_parameter("battery_topic", "bat_vol");
  bat_topic_ = get_parameter("battery_topic").get_parameter_value().get<std::string>();
  declare_parameter("cmd_vel_topic", "cmd_vel");
  cmd_vel_topic_ = get_parameter("cmd_vel_topic").get_parameter_value().get<std::string>();

  //参数初始化
  declare_parameter("robot_port", "/dev/ttyS0");
  serial_port_ = get_parameter("robot_port").get_parameter_value().get<std::string>();
  declare_parameter("robot_port_baud", 230400);
  serial_port_baud_ = get_parameter("robot_port_baud").get_parameter_value().get<int>();
  declare_parameter("pub_odom_tf", true);
  pub_odom_tf_ = get_parameter("pub_odom_tf").get_parameter_value().get<bool>();

  //车型选择参数
  declare_parameter("robot_type_send", "r20_fwd");
  robot_type_send_ = get_parameter("robot_type_send").get_parameter_value().get<std::string>();

  //模式选择
  declare_parameter("light_mode", 0x02);
  RGB_M1_ = get_parameter("light_mode").get_parameter_value().get<int>();
  declare_parameter("light_mode2", 0);
  RGB_M2_ = get_parameter("light_mode2").get_parameter_value().get<int>();
  declare_parameter("light_time", 0);
  RGB_T_ = get_parameter("light_time").get_parameter_value().get<int>();
  declare_parameter("rgb_r", 0x00);
  RGB_R_ = get_parameter("rgb_r").get_parameter_value().get<int>();
  declare_parameter("rgb_g", 0xFF);
  RGB_G_ = get_parameter("rgb_g").get_parameter_value().get<int>();
  declare_parameter("rgb_b", 0x00);
  RGB_B_ = get_parameter("rgb_b").get_parameter_value().get<int>();
  ...
  bat_pub_ = create_publisher<std_msgs::msg::Float32>(bat_topic_, rclcpp::SensorDataQoS());

  //实例化订阅者对象
  cmd_vel_sub_ = create_subscription<geometry_msgs::msg::Twist>(
    cmd_vel_topic_, 10,
    std::bind(&TarkbotRobot::cmdVelCallback, this, std::placeholders::_1));
  // cmd_vel_sub_ = nh_.subscribe<geometry_msgs::Twist>(
  //   cmd_vel_topic_, 100,
  //   &TarkbotRobot::cmdVelCallback, this);
  ...
  //提示信息，串口端口号和波特率
  RCLCPP_INFO(
    this->get_logger(), "Tarkbot Robot Set serial %s at %d baud",
  ...
  RCLCPP_INFO(
    this->get_logger(),
    "Tarkbot Robot Set color_mode:%d, color_R:%d, color_G:%d, color_B:%d", RGB_M1_, RGB_R_,
    RGB_G_, RGB_B_);

  //向底层发送车型数据
  static uint8_t cartype_data[1];
  if (robot_type_send_ == "r20x_mec") {
    cartype_data[0] = 1;
  }
  if (robot_type_send_ == "r20x_fwd") {
    cartype_data[0] = 2;
  }
  if (robot_type_send_ == "r20x_akm") {
    cartype_data[0] = 3;
  }
  if (robot_type_send_ == "r20x_twd") {
    cartype_data[0] = 4;
  }
  if (robot_type_send_ == "r20x_tak") {
    cartype_data[0] = 5;
  }
  if (robot_type_send_ == "r20x_omni") {
    cartype_data[0] = 6;
  }
  sendSerialPacket(cartype_data, 1, ID_ROS2CTR_RTY);
  RCLCPP_INFO(this->get_logger(), "Tarkbot Robot Set car type %s", robot_type_send_.c_str());

  //提示信息，发布订阅的话题消息
  RCLCPP_INFO(
    this->get_logger(), "Tarkbot Robot setup publisher  on odom [%s]",
    odom_topic_.c_str());
  RCLCPP_INFO(
    this->get_logger(), "Tarkbot Robot setup publisher  on imu [%s]",
    imu_topic_.c_str());
  RCLCPP_INFO(
    this->get_logger(), "Tarkbot Robot setup publisher  on bat_vol [%s]",
    bat_topic_.c_str());
  RCLCPP_INFO(
    this->get_logger(), "Tarkbot Robot setup subscriber on cmd_vel [%s]",
...
TarkbotRobot::~TarkbotRobot()
{
  ...
  //提示信息
  RCLCPP_INFO(this->get_logger(), "Tarkbot Robot is Closed!");
}
...
void TarkbotRobot::light_callback(const std_msgs::msg::ColorRGBA::ConstPtr & light)
{
...
void TarkbotRobot::recvDataHandle(uint8_t * buffer_data)
{
  ...
  if (buffer_data[3] == ID_CPR2ROS_DATA) {
    ...
    //计算里程计数据
    pos_data_.pos_x +=
      (vel_data_.linear_x * cos(pos_data_.angular_z) - vel_data_.linear_y *
      sin(pos_data_.angular_z)) * DATA_PERIOD;
    pos_data_.pos_y +=
      (vel_data_.linear_x * sin(pos_data_.angular_z) + vel_data_.linear_y *
      cos(pos_data_.angular_z)) * DATA_PERIOD;
...
{
  //获取数据
  imu_msgs_.header.stamp = get_clock()->now();
 ...
 */
void TarkbotRobot::cmdVelCallback(const geometry_msgs::msg::Twist::ConstPtr & msg)
{
  static uint8_t vel_data[11];
...
void TarkbotRobot::SetColor(float r, float g, float b, float a, uint8_t selet)
{
  ...
  RCLCPP_INFO(
    this->get_logger(),
    "Light Control : selet: %d R: %d, G: %d, B: %d", selet, send_buff[6], send_buff[7],
    send_buff[8]);
}
...
//     RCLCPP_INFO(
//       this->get_logger(), "Calibrating the IMU, Please hold the robot stationary for 5 seconds. ");
...
//   //发送常亮灯光控制
//   if ((config.light_chose != RGB_M1_) || (config.RGB_R != RGB_R_) || (config.RGB_G != RGB_G_) ||
//     (config.RGB_G != RGB_B_))
//   {
...
//     RCLCPP_INFO(
//       this->get_logger(),
//       "Tarkbot Robot Set color_mode_reconfig:%d, color_R:%d, color_G:%d, color_B:%d",
//       config.light_chose, config.RGB_R, config.RGB_G, config.RGB_B);
//   }
// }
...
void TarkbotRobot::calculateImuQuaternion(struct imu_data imu_cel)
{
  ...
  // RCLCPP_INFO(this->get_logger(), "IMU:%f  %f  %f ",roll,pitch,yaw);
