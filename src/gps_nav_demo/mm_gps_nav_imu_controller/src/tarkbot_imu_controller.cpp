...
class TarkbotImuEntity : public aros2_hardware::HardwareEntity<sensor_msgs::msg::Imu>
{
public:
  void on_activate() override
  {
    RCLCPP_INFO(get_node()->get_logger(), "TarkbotImuEntity on_activate() is called.");
    ...
    RCLCPP_INFO(get_node()->get_logger(), "TarkbotImuEntity on_activate() is finished.");
  }

  void on_deactivate() override
  {
    RCLCPP_INFO(get_node()->get_logger(), "TarkbotImuEntity on_deactivate() is called.");
    imu_sub_.reset();
    RCLCPP_INFO(get_node()->get_logger(), "TarkbotImuEntity on_deactivate() is finished.");
  }
...
int main(int argc, char ** argv)
{
  ...
  auto node =
    std::make_shared<mm_gps_nav_imu_controller::ImuController<mm_gps_nav_imu_controller::TarkbotImuEntity>>();
  auto fake_imu_entity_ = std::make_shared<mm_gps_nav_imu_controller::TarkbotImuEntity>();
